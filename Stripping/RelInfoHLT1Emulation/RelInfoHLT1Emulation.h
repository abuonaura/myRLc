 ...
 *
 * \brief Calculate informations from two-particle combinations of candidate's daughters.
 *      Describe better here what the tool does
 *    
 *
 * Variables:
 * - NDAUGHTERS             : Number of stable daughters found in the decay. maximum saved is MAXNUMBER
 * - NCOMBINATIONS          : Number of combinations. Maximum is [MAXNUMBER*(MAXNUMBER-1)]/2
 ...
 * - MCORR_OWNPV_COMB_i_j   : Corrected Mass of the i-j combination using the OWNPV
 ...
 * 
 *  @author Simone Meloni (simone.meloni@cern.ch)
 *  @date   5/17/2019
 *
 */

class RelInfoHLT1Emulation : public GaudiTool, virtual public IRelatedInfoTool
{

public:

  /// Standard constructor
  RelInfoHLT1Emulation( const std::string& type,
...
private:
  ...
  void setTransverseMomenta(const std::vector<const LHCb::Particle*> &particlesVector,	
  ...
  void setIPChi2(const std::vector<const LHCb::Particle*> &particlesVector,
  							 std::vector<float> &valuesVector);
  void setTrackGhostProb(const std::vector<const LHCb::Particle*> &particlesVector,
  											 std::vector<float> &valuesVector);
  void setTrackChi2(const std::vector<const LHCb::Particle*> &particlesVector,
  									std::vector<float> &valuesVector);
  void setTrackNDOF(const std::vector<const LHCb::Particle*> &particlesVector,
  									std::vector<float> &valuesVector);
  ...
  void setDaughtersSumPT(const std::vector<const LHCb::Particle*> &particlesVector,
                         std::vector<float> &valuesVector);
...
private:
  ...
  inline bool isBasicParticle(const LHCb::Particle* P)const
  {
  	...
  	bool hasproto = fabs(PID) != 22 && fabs(PID) != 111 && fabs(PID) != 221;
...
private: 
  ...
  std::vector< const LHCb::Particle*> m_decayParticles;  // All the stable daughters
  std::vector< const LHCb::Particle*> m_motherParticles; // All the two-particle combinations
  std::vector< const LHCb::Vertex*> m_motherVertices;    // All the vertices of the two-particle combinations

private:
  // Variables you want to save in the RelatedInfoMap
  int m_NDAUGHTERS; // Number of stable daughters
  int m_NCOMBINATIONS; // Number of two-particle combinations of stable daughters
  ...
  std::vector<float> m_DAUGHTERSIPCHI2BEST; // IPCHI2 wrt OWNPV of each stable daughter
  std::vector<float> m_DAUGHTERSTRACKGHOSTPROB; // Ghost Prob of each stable daughter 
  std::vector<float> m_DAUGHTERSTRACKCHI2; // Track Chi2 of each stable daughter 
  std::vector<float> m_DAUGHTERSTRACKNDOF; // Track NDOF of each stable daughter
  ...
  std::vector<float> m_COMBINATIONSMCORR_BPV; // Corrected mass of the combinations using the OWNPV
  std::vector<float> m_COMBINATIONSBVVDCHI2; // VDCHI2 of each combination wrt OWNPV
  std::vector<float> m_COMBINATIONSSUMPT; // Sum of the PT of the combinations daughters
  std::vector<float> m_COMBINATIONSDIRA_BPV; // Dira wrt the flight direction from OWNPV
  std::vector<float> m_COMBINATIONSDOCACHI2; // DOCA of the daughters for each combination
  std::vector<float> m_COMBINATIONSBPVIPCHI2; // IPCHi2 of the combinations wrt OWNPV
  std::vector<float> m_COMBINATIONSNLT16_BPV; //NLT16 variable, i.e. number of stable daughters with IPCHI2 < 16
