...
RooStats::ModelConfig* SysFit::SetChannelConstants(RooStats::ModelConfig *mc, string channel)
{
	...
	((RooRealVar*)(mc->GetNuisanceParameters()->find(("mcNc2charm_mbody_"+channel).c_str())))->setConstant(kTRUE);
	((RooRealVar*)(mc->GetNuisanceParameters()->find(("mcNc2charm_2body_"+channel).c_str())))->setConstant(kTRUE);
...
{       
	...
	for(Int_t i=0; i<nchannels; i++)
	{
		...
		for(Int_t j=0; j<category.size(); j++)
		{
			...
			cout<<"--------------------------------------------"<<endl;
                        cout<<"Is category : "<<category[j]<<"  shape uncertain? "<< IsShapeUncertain(category[j])<<endl;
                        cout<<"Is category : "<<category[j]<<"  Gauss constrained? "<< IsGaussConstrained(category[j])<<endl;
                        cout<<"--------------------------------------------"<<endl;
			if(category[j]=="pha")
				continue;
			if(category[j]!="tau"&&category[j]!="2charm")
				AddSample(category[j],filename,0,IsGaussConstrained(category[j]),BBeast,&channels[i],start_parameters[category[j]]);
			if(category[j]=="2charm")
			{
				AddSample(category[j]+"_2body",filename,IsShapeUncertain(category[j]+"_2body"),IsGaussConstrained(category[j]+"_2body"),BBeast,&channels[i],start_parameters[category[j]]);
				AddSample(category[j]+"_mbody",filename,IsShapeUncertain(category[j]+"_mbody"),IsGaussConstrained(category[j]+"_mbody"),BBeast,&channels[i],start_parameters[category[j]]);

			}
			if(category[j]=="tau")//NB:: To modify the 0 adding ShapeUnc
				AddSample(category[j],filename,0,IsGaussConstrained(category[j]),BBeast,&channels[i],start_parameters[category[j]],start_parameters["mu"]);
		}
...
vector<Double_t> SysFit::Fit(RooStats::ModelConfig* mc, RooStats::HistFactory::Measurement meas, RooWorkspace *w)
{
