...
void SysFit::SetStartParameter(string channel, string sample, Double_t mean, Double_t min, Double_t max)
{
	vector<Double_t> value = {mean, min, max};
	start_parameters[channel].insert(pair<string, vector<Double_t>>("N_"+sample+"_"+channel,value));
}

void SysFit::SetTemplateFileName(string ch, string name)
{
	template_fname[ch] = name;
}

void SysFit::SetSampleNames(TString MCcat)
{
	if(MCcat=="MCfull")
		sample_names = {"Lb_Lcmunu","Lb_Lctaunu","Lb_LcDs-2body","Lb_LcDs-mbody","Lb_Lc2625munu","Lb_Lc2625taunu","Lb_Lc2625Ds-2body","Lb_Lc2625Ds-mbody","Lb_Lc2593munu","Lb_Lc2593taunu","Lb_Lc2593Ds-2body","Lb_Lc2593Ds-mbody","MISID","Combinatorial"};
	if(MCcat=="MCTrackerOnly")
		sample_names = {"Lb_Lcmunu","Lb_Lctaunu","Lb_LcDs-2body","Lb_LcDs-mbody","Lb_Lc2625munu","Lb_Lc2625taunu","Lb_Lc2625Ds-2body","Lb_Lc2625Ds-mbody","Lb_Lc2593munu","Lb_Lc2593taunu","Lb_Lc2593Ds-2body","Lb_Lc2593Ds-mbody","B_Lcpbarmunu","Lb_Lc2880munu","Lb_Lc2765munu","MISID","Combinatorial"};
}

//-------------------------------------------------------------------------------------------------------------------
//Get Fit configuration functions
//-------------------------------------------------------------------------------------------------------------------
...
vector<Double_t> SysFit::GetStartParameter(string ch_name, string spl_name)
{
	string paramname = "N_"+spl_name+"_"+ch_name;
	vector<Double_t> params = start_parameters[ch_name][paramname];
	return params;
}

string SysFit::GetTemplateFileName(string ch)
{
	return template_fname[ch];
}
...
Double_t SysFit::GetHistoNormalisation(string inputFile, string type)
{
	...
	Double_t Norm = 1./h_temp->Integral();

	cout<<" Integral_"+type+" = "<<h_temp->Integral()<<endl;
	cout<<" Normalisation factor "+type+" = " << Norm <<endl;
...
void SysFit::CreateSweightCorrectHistos(string inputFile, string newfilename, vector<string> snames)
{
    //Read root file with histograms of samples
	...
	//Loop over all the other samples entering the fit
	for(Int_t j=0; j<snames.size(); j++)
    {
        //Get the histo from the file and save in htemp
        if (snames[j]!="Lb_LcDs-mbody"&&snames[j]!="Lb_Lc2625Ds-mbody"&&snames[j]!="Lb_Lc2593Ds-mbody")
        {
            TH3* h_temp;
            oldhistFile->GetObject(("h_"+snames[j]).c_str(),h_temp);
            if(h_temp==NULL)
        ...
        else
        {
            ...
            h_temp->push_back((TH3*)oldhistFile->Get(("h_"+snames[j]).c_str()));
            for(Int_t a = 0; a<suffix.size();a++)
            {
                h_temp->push_back((TH3*)oldhistFile->Get(("h_"+snames[j]+"_"+suffix[a]).c_str()));
            }
...
void SysFit::AddSample(string type, string inputFile, bool GaussConstraints, const bool BBeast, Channel** chan)
{
	//Define the sample
    Sample sample;
    ...
    Double_t normfactor = GetHistoNormalisation(MCnormFile, type);

	//Get the name of the channel (Isolated, Kenriched or Lcpipi)
	...
	if(type=="Lb_LcDs-mbody" || type=="Lb_Lc2625Ds-mbody"|| type=="Lb_Lc2593Ds-mbody")
	{
		sample.AddHistoSys(type+"_lin_var","h_"+type+"_1ml",TemplateFile,"", "h_"+type+"_1pl",TemplateFile,"");
        sample.AddHistoSys(type+"_quad_var","h_"+type+"_1mq",TemplateFile,"", "h_"+type+"_1pq",TemplateFile,"");
	...
	sample.AddNormFactor("norm_"+type+"_"+category,normfactor,1e-20,1.);

	//Add additional normalisation factors
	if(type=="Lb_Lctaunu")
	{
		//normalisation to the start params of the Lc muon sample
		vector<Double_t> muparam = GetStartParameter(category,"Lb_Lcmunu");
		sample.AddNormFactor("N_Lb_Lcmunu_"+category, muparam[0], muparam[1], muparam[2]); 
		//moltiplication for the RLc factor
		vector<Double_t> param = GetStartParameter(category,"Lb_Lctaunu");
		sample.AddNormFactor("RLc_"+category, param[0],param[1],param[2]); //times the value of RLc
	}
	else if(type=="Lb_Lc2625taunu")
    {
		//normalisation to the start params of the Lc muon sample
		vector<Double_t> muparam = GetStartParameter(category,"Lb_Lc2625munu");
        sample.AddNormFactor("N_Lb_Lc2625munu_"+category, muparam[0],muparam[1],muparam[2]); 
		//moltiplication for the RLc factor
		vector<Double_t> param = GetStartParameter(category,"Lb_Lc2625taunu");
        sample.AddNormFactor("RLcStarHigh_"+category, param[0],param[1],param[2]); //times the value of RLc star
    }
	else if(type=="Lb_Lc2593taunu")
    {
		//normalisation to the start params of the Lc muon sample
		vector<Double_t> muparam = GetStartParameter(category,"Lb_Lc2593munu");
        sample.AddNormFactor("N_Lb_Lc2593munu_"+category, muparam[0],muparam[1],muparam[2]); 
		//moltiplication for the RLc factor
		vector<Double_t> param = GetStartParameter(category,"Lb_Lc2593taunu");
        sample.AddNormFactor("RLcStarLow_"+category, param[0],param[1],param[2]); //times the value of RLc star
    }
	else
	{
		//normalisation to starting parameters (for all other samples)
		vector<Double_t> param = GetStartParameter(category,type);
		sample.AddNormFactor("N_"+type+"_"+category, param[0],param[1],param[2]); 
	}
...
{
	//Set RLc constant and practically 0 for both Kenriched and Lcpipi channel categories
	if(channel=="Kenriched" || channel=="Lcpipi")
	{
		((RooRealVar*)(mc->GetParametersOfInterest()->find(("RLc_"+channel).c_str())))->setVal(1E-15);
        ((RooRealVar*)(mc->GetParametersOfInterest()->find(("RLc_"+channel).c_str())))->setConstant(kTRUE);
	}
	//For the Kenriched channel set constant also RLcStar (both high and low)
	if (channel=="Kenriched")
	{
		((RooRealVar*)(mc->GetParametersOfInterest()->find(("RLcStarHigh_"+channel).c_str())))->setVal(1E-15);
		((RooRealVar*)(mc->GetParametersOfInterest()->find(("RLcStarHigh_"+channel).c_str())))->setConstant(kTRUE);
		((RooRealVar*)(mc->GetParametersOfInterest()->find(("RLcStarLow_"+channel).c_str())))->setVal(1E-15);
		((RooRealVar*)(mc->GetParametersOfInterest()->find(("RLcStarLow_"+channel).c_str())))->setConstant(kTRUE);
	...
	//For all the channels set set the normalisation factors to constant
	vector<string> snames = GetSampleNames();
	for(Int_t i =0; i<snames.size();i++)
		((RooRealVar*)(mc->GetNuisanceParameters()->find(("norm_"+snames[i]+"_"+channel).c_str())))->setConstant(kTRUE);
...
{       
	...
	vector <string> channel_names = GetChannels2fit();	
	TString MCcat = GetMCcategory();
	SetSampleNames(MCcat);
	...
	for(Int_t i=0; i<nchannels; i++)
	{
		...
		if(IsSweightCorrOn())
        {
            ...
            CreateSweightCorrectHistos(filename, newfilename, snames);
        }
		...
		for(Int_t j=0; j<snames.size(); j++)
		{
			...
			AddSample(snames[j], filename, 0,BBeast,&channels[i]);
			//TO IMPLEMENT POSSIBILITY TO ADD GAUSS CONSTRAINTS
		}
		...
		meas.SetPOI("RLc_"+channel_names[i]);
		meas.SetPOI("RLcStarHigh_"+channel_names[i]);
		meas.SetPOI("RLcStarLow_"+channel_names[i]);
...
{
	((RooRealVar*)(mc->GetNuisanceParameters()->find(("N_"+sample+"_"+channel).c_str())))->setConstant(kTRUE);
	return mc;
}
...
{
	((RooRealVar*)(mc->GetNuisanceParameters()->find("alpha_Lb_LcDs-mbody_lin_var")))->setVal(0.5);
	((RooRealVar*)(mc->GetNuisanceParameters()->find("alpha_Lb_LcDs-mbody_quad_var")))->setVal(0.5);
	((RooRealVar*)(mc->GetNuisanceParameters()->find("alpha_Lb_Lc2625Ds-mbody_lin_var")))->setVal(0.5);
	((RooRealVar*)(mc->GetNuisanceParameters()->find("alpha_Lb_Lc2625Ds-mbody_quad_var")))->setVal(0.5);
	((RooRealVar*)(mc->GetNuisanceParameters()->find("alpha_Lb_Lc2593Ds-mbody_lin_var")))->setVal(0.5);
	((RooRealVar*)(mc->GetNuisanceParameters()->find("alpha_Lb_Lc2593Ds-mbody_quad_var")))->setVal(0.5);
	return mc;
}
...
RooFitResult* SysFit::Fit(RooStats::ModelConfig* mc, RooStats::HistFactory::Measurement meas, RooWorkspace *w, Bool_t plotonly, Bool_t refit)
{
	...
	cout << "------------------Printing fit nuisance parameters starting values ------------------" << endl;
	...
	if(plotonly||refit)
	{
		for(Int_t i=0; i<nchannels;i++)
		{
			string resultfname = "results/FitResultsUnblinded_"+channel_names[i]+".txt";
	...
	RooRealVar *alpha_LcDs_lin;
	RooRealVar *alpha_LcDs_quad;
	RooRealVar *alpha_Lc2593Ds_lin;
	RooRealVar *alpha_Lc2593Ds_quad;
	RooRealVar *alpha_Lc2625Ds_lin;
	RooRealVar *alpha_Lc2625Ds_quad;
	...
	alpha_LcDs_lin = (RooRealVar*) mc->GetNuisanceParameters()->find("alpha_Lb_LcDs-mbody_lin_var");
	alpha_LcDs_quad = (RooRealVar*) mc->GetNuisanceParameters()->find("alpha_Lb_LcDs-mbody_quad_var");
	alpha_Lc2593Ds_lin = (RooRealVar*) mc->GetNuisanceParameters()->find("alpha_Lb_Lc2593Ds-mbody_lin_var");
	alpha_Lc2593Ds_quad = (RooRealVar*) mc->GetNuisanceParameters()->find("alpha_Lb_Lc2593Ds-mbody_quad_var");
	alpha_Lc2625Ds_lin = (RooRealVar*) mc->GetNuisanceParameters()->find("alpha_Lb_Lc2625Ds-mbody_lin_var");
	alpha_Lc2625Ds_quad = (RooRealVar*) mc->GetNuisanceParameters()->find("alpha_Lb_Lc2625Ds-mbody_quad_var");
	...
	for (Int_t i =0;i< nchannels;i++)
	{
		...
		//Put together the two RLcStar values
		RooRealVar *RLcSt_high, *RLcSt_low;
		RooRealVar *RLcStar = new RooRealVar(("RLcStar_"+channel_names[i]).c_str(),
				("RLcStar_"+channel_names[i]).c_str(), 0, 0., 1.);


		RLcSt_high = (RooRealVar*) mc->GetParametersOfInterest()->find(("RLcStarHigh_"+channel_names[i]).c_str());
		RLcSt_low = (RooRealVar*) mc->GetParametersOfInterest()->find(("RLcStarLow_"+channel_names[i]).c_str());

		if(plotonly==false)
		{
			//Set the linear alphas for the LcXc and LcXcStar templates to be one parameter
			cust->replaceArg(*alpha_LcDs_lin,*alpha_linear);
			cust->replaceArg(*alpha_Lc2593Ds_lin,*alpha_linear);
			cust->replaceArg(*alpha_Lc2625Ds_lin,*alpha_linear);
			//Set the quadratic alphas for the LcXc and LcXcStar templates to be one parameter
			cust->replaceArg(*alpha_LcDs_quad,*alpha_quadratic);
			cust->replaceArg(*alpha_Lc2593Ds_quad,*alpha_quadratic);
			cust->replaceArg(*alpha_Lc2625Ds_quad,*alpha_quadratic);
			//Set the two RLcStar values to be one parameter for Lcpipi and Isolated fits
			if(RLcSt_high && RLcSt_low)
			{
				if(channel_names[i]=="Kenriched")
				{
					RLcStar->setVal(1E-16);
					RLcStar->setConstant(kTRUE);
				}
				cust->replaceArg(*RLcSt_high, *RLcStar);
				cust->replaceArg(*RLcSt_low , *RLcStar);
...
{
    RooRealVar* RLc_fitresult = (RooRealVar*)fitResult->floatParsFinal().find("RLc_"+TString(name_suffix));
	if(RLc_fitresult)
	{
	 ...
	 RooRealVar* RLcStar_fitresult = (RooRealVar*)fitResult->floatParsFinal().find("RLcStar_"+TString(name_suffix));
...
//-------------------------------------------------------------------------------------------------------------------
//Functions to save/load fit results
//-------------------------------------------------------------------------------------------------------------------


ModelConfig* SysFit::LoadFitResults(string fname, ModelConfig* mc, string channelName)
{
	ifstream infile;
	infile.open(fname);
	string search="RLc_"+channelName;
	cout<< "string search: "<<search<<endl;
	string search2="RLcStar_"+channelName;
	...
	{
		if(name.compare(search)!=0 && name.compare(search2)!=0 && name.compare(search3)!=0 
		&& name.compare(search4)!=0)
		{	
			cout<<name<<endl;
		...
		else if (name.compare(search2)==0)
		{
			((RooRealVar*)(mc->GetParametersOfInterest()->find(("RLcStarHigh_"+channelName).c_str())))->setVal(value);
			((RooRealVar*)(mc->GetParametersOfInterest()->find(("RLcStarLow_"+channelName).c_str())))->setVal(value);
		}

		else if(name.compare(search3)==0)
		{	
			((RooRealVar*)(mc->GetNuisanceParameters()->find("alpha_Lb_LcDs-mbody_lin_var")))->setVal(value);
			((RooRealVar*)(mc->GetNuisanceParameters()->find("alpha_Lb_Lc2625Ds-mbody_lin_var")))->setVal(value);
			((RooRealVar*)(mc->GetNuisanceParameters()->find("alpha_Lb_Lc2593Ds-mbody_lin_var")))->setVal(value);
		}
		else if(name.compare(search4)==0)
		{	
			((RooRealVar*)(mc->GetNuisanceParameters()->find("alpha_Lb_LcDs-mbody_quad_var")))->setVal(value);
			((RooRealVar*)(mc->GetNuisanceParameters()->find("alpha_Lb_Lc2625Ds-mbody_quad_var")))->setVal(value);
			((RooRealVar*)(mc->GetNuisanceParameters()->find("alpha_Lb_Lc2593Ds-mbody_quad_var")))->setVal(value);
		}
	}
	return mc;
}
...
{
	TString name = "";
	if (component.Contains("_Lb_Lcmunu_")) name = "#Lambda_{b} #rightarrow #Lambda_{c} #mu #nu";
	else if(component.Contains("h_Lb_Lc2593munu_")) name = "#Lambda_{b} #rightarrow #Lambda_{c}*(2593) #mu #nu_{#mu}";
	else if(component.Contains("h_Lb_Lc2625munu_")) name = "#Lambda_{b} #rightarrow #Lambda_{c}*(2625) #mu #nu_{#mu}";
	else if(component.Contains("h_Lb_Lc2593Ds-2body")) name = "#Lambda_{b} #rightarrow #Lambda_{c}*(2593) X_{c}";
	else if(component.Contains("h_Lb_Lc2625Ds-2body")) name = "#Lambda_{b} #rightarrow #Lambda_{c}*(2625) X_{c}";
	else if(component.Contains("h_Lb_Lc2593Ds-mbody")) name = "#Lambda_{b} #rightarrow #Lambda_{c}*(2593) X_{c} X";
	else if(component.Contains("h_Lb_Lc2625Ds-mbody")) name = "#Lambda_{b} #rightarrow #Lambda_{c}*(2625) X_{c} X";
	else if(component.Contains("h_Lb_Lc2593taunu_")) name = "#Lambda_{b} #rightarrow #Lambda_{c}*(2593) #tau #nu_{#tau}";
	else if(component.Contains("h_Lb_Lc2625taunu_")) name = "#Lambda_{b} #rightarrow #Lambda_{c}*(2625) #tau #nu_{#tau}";
	else if(component.Contains("h_Lcpbar_")) name = "#B #rightarrow #Lambda_{c} #bar{p} #mu_{#mu} #nu_{#tau}";
	else if(component.Contains("h_Lb_Lctaunu_")) name = "#Lambda_{b} #rightarrow #Lambda_{c} #tau #nu_{#tau}";
	else if(component.Contains("h_Lb_LcDs-2body")) name = "#Lambda_{b} #rightarrow #Lambda_{c} X_{c}";
	else if(component.Contains("h_Lb_LcDs-mbody")) name = "#Lambda_{b} #rightarrow #Lambda_{c} X_{c} X";
...
{       
	Int_t color=-99;
	if (component.Contains("_Lb_Lcmunu_")) color = kBlue;
	else if(component.Contains("h_Lb_Lc2593munu_")) color = kAzure+2;
	else if(component.Contains("h_Lb_Lc2625munu_")) color = kAzure-8;
	else if(component.Contains("h_Lb_Lc2593Ds-2body")) color = kViolet;
	else if(component.Contains("h_Lb_Lc2625Ds-2body")) color = kViolet-1;
	else if(component.Contains("h_Lb_Lc2593Ds-mbody")) color = kViolet+6;
	else if(component.Contains("h_Lb_Lc2625Ds-mbody")) color = kViolet+7;
	else if(component.Contains("h_Lb_LcDs-2body")) color = kGreen;
	else if(component.Contains("h_Lb_LcDs-mbody")) color = kGreen+3;
	else if(component.Contains("h_Lb_Lc2593taunu_")) color = kRed+2;
	else if(component.Contains("h_Lb_Lc2625taunu_")) color = kRed+1;
	else if(component.Contains("h_Lb_Lctaunu_")) color = kRed;
	else if(component.Contains("h_Lcpbar_")) color = kGreen+1;
...
void SysFit::PlotFitVariables(RooRealVar* fitvar1,const char* title1, RooRealVar* fitvar2, const char* title2, RooRealVar* fitvar3, const char* title3,std::map<std::string,RooDataHist*> data,RooSimultaneous* model, RooCategory* idx,string name_suffix)
{
	...
	c1->Print(TString("plots_")+GetMCcategory()+TString("/FitVariables_")+TString(name_suffix)+TString("_")+GetFitType()+TString(".pdf"));
...
void SysFit::PlotFrame(RooRealVar* kinemObserv,const char* title,RooAbsData* data,RooSimultaneous* model, RooCategory* idx,Double_t plotStart, Double_t plotEnd, const char* units,string name_suffix, bool legend=kFALSE)
{
	...
	if (strcmp(title,"q^{2}")==0)
	{
		...
		leg.Draw();
		clegend->Print(TString("plots_")+GetMCcategory()+TString("/Legend.pdf"));
	}
	...
	c1->Print(TString("plots_")+GetMCcategory()+TString("/Fit_")+GetFitVarName(title)+TString("_")+TString(name_suffix)+TString("_")+GetFitType()+TString(".pdf"));
