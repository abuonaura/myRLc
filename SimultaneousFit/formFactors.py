...
def defParamGener(generPoints, typ,count=0):
        ...
        if typ=='g0': deltaF = 0
        #print(generPoints.keys())
        for iK in generPoints.keys():
            #print(iK.decode('utf-8'),typ)
            if iK.decode('utf-8')=='xa0'+typ: a0 = generPoints[iK][count]
            if iK.decode('utf-8')=='xa1'+typ: a1 = generPoints[iK][count]
        if typ=="gt": a0 = generPoints[b"xa0gp"][count]
        m_pole = m_Bc + deltaF
        tp = m_pole*m_pole
...
def plotManyTemplates(plot=1):
    ...
    hTauList, hMuList = [],[]
    for iSampl in range(len(paramSample[list(paramSample.keys())[0]])):
    ...
    if plot:
        ...
        c.Divide(1,2)
        c.cd(1)
        for iH in range(20):#len(hTauList)):
...
def makeDistrPerQ2():
    ...
    hTauQ2,hMuQ2 = {},{}
    for iQ in range(hTauList[0].FindFirstBinAbove(),hTauList[0].FindLastBinAbove()+1):
    ...
    for iQ in range(hMuList[0].FindFirstBinAbove(),hMuList[0].FindLastBinAbove()+1):
    ...
    for iH in hTauList:
        for iQ in range(iH.FindFirstBinAbove(),iH.FindLastBinAbove()+1):
