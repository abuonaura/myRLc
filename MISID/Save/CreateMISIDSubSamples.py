'''
    ...
    Description: Creates MISID templates
'''
import ROOT as r
import numpy as np
...
def AddSweights(ifile,polarity,particle,sample):
    ...
    of_fit = r.TFile('FitResults/FitResults_'+particle+'_'+polarity+ suffix[sample],'Recreate')
    result.Write()
    of_fit.Close()
...
def AddMISIDweightNoCF(ifile, ofile,sample,polarity, particle):
    ...
    #hID.SetName('hID_'+particle+'_'+polarity)
    print(hID.GetName(),hID.GetEntries())
    hMuMISID = GetPIDhisto(pathMisid['Mu']+fhistname,hnameMuMisID)
    #hMuMISID.SetName('hMuMISID_'+particle+'_'+polarity)
    print(hMuMISID.GetName(),hMuMISID.GetEntries())
    hHadMISID = GetPIDhisto(pathMisid[particle]+fhistname,hnameHadMisID)
    #hHadMISID.SetName('hHadMISID_'+particle+'_'+polarity)
...
if __name__ == '__main__':
    ...
    if sample=='Kenriched':
        files = {'MagUp': datadir+'ControlSamples/Lb_FakeMu_MagUp_reduced_preselected'+suffix[sample][0:-5]+'_sw.root',
                'MagDown': datadir+'ControlSamples/Lb_FakeMu_MagDown_reduced_preselected'+suffix[sample][0:-5]+'_sw.root'}
    else:
        files = {'MagUp': datadir+'Data/Lb_FakeMu_MagUp_reduced_preselected'+suffix[sample][0:-5]+'_sw.root',
            'MagDown': datadir+'Data/Lb_FakeMu_MagDown_reduced_preselected'+suffix[sample][0:-5]+'_sw.root'}
    print ('Starting files: ', files)

    for polarity in polarities:
        for particle in particles:
            outfile =  datadir+'MISID/OppositeSign/'+particle+'_sample_'+polarity+suffix[sample]
