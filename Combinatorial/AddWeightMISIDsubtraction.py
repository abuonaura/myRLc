...
h_muMISID_Pi = r.TH3F('h_muMISID_Pi',';E_{#mu} (MeV);q^{2} (MeV^{2});Mmiss^{2} (MeV^{2}',10,0,2600,4,-2E6,14E6,10,-2E6,14E6)

h_muMISID_K = r.TH3F('h_muMISID_K',';E_{#mu} (MeV);q^{2} (MeV^{2});Mmiss^{2} (MeV^{2}',10,0,2600,4,-2E6,14E6,10,-2E6,14E6)
...
def ComputeMISIDFractions(filenameIN, filenameOUT):
    ...
    for i in range(h_fPi.GetNbinsX()):
        for j in range(h_fPi.GetNbinsY()):
            for k in range(h_fPi.GetNbinsZ()):
                print(h_fPi.GetBinContent(i,j,k))
    #h_fK = fraction of kaons misidentified as muons in each bin
    h_fK = r.TH3F('h_fK',';E_{#mu} (MeV); q^{2} (MeV^{2}); Mmiss^{2} (MeV^{2});',10,0,2600,4,-2E6,14E6,10,-2E6,14E6)
...
def AddMISIDweights(ifile,ofile, fractionfile,sample):
    ...
    for nevt in range(ot.GetEntries()):
        ...
        #print(h_fK.GetBinContent(ibinK), h_fPi.GetBinContent(ibinPi))
